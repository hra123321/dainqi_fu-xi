...
## 2026-07-09 长期平台阶段 4：错题能力画像

- 使用 Skill：`knowledge-first`。
- 输入范围：错题日志、错题本服务、错题本 API。
- 结果：新增能力画像服务，按学科、知识点、题型难度和错误类型生成薄弱能力图。
- 注意：画像先基于本地 JSONL 错题日志计算，不改动题目生成和批改链路，降低回归风险。
## 2026-07-09 长期平台阶段 5：多端同步 API 骨架

- 使用 Skill：`knowledge-first`。
- 输入范围：移动端入口、领域服务、能力画像、SQLite 本地库。
- 结果：新增 `/api/v1/sync/push` 与 `/api/v1/sync/pull`，支持 Android 离线事件上传和 Windows 主库状态下发。
- 注意：第一版同步协议只处理事件与只读状态下发，不在 Android 保存 DeepSeek Key，后续再加冲突解决和文件分片同步。
